...
function drawWeeklyChart(bins) {
  ...
  bins.forEach((val, i) => {
    ...
    g.addColorStop(0, "#00d38a"); g.addColorStop(1, "#00ffae");
    ctx.fillStyle = g; ctx.shadowColor = "#00ffae"; ctx.shadowBlur = 10;
    ctx.fillRect(x, y, barWidth, barHeight);
    ctx.shadowBlur = 0; ctx.fillStyle = "#f3f4f6"; ctx.textAlign = "center"; ctx.font = "14px Segoe UI";
...
async function loadHome() {
  ...
  animateValue(totalPointsEl, 0, pts.totalPoints, 1200);
  animateValue(weeklyPointsEl, 0, pts.weeklyPoints, 1200);
  animateValue(dailyPointsEl, 0, pts.dailyPoints, 1200);
  animateValue(streakEl, 0, pts.streak || 0, 1200);
  drawWeeklyChart(pts.hist || [0,0,0,0,0,0,0]);
}

bossArt.addEventListener("mouseenter", () => { bossArt.style.filter = "drop-shadow(0 0 20px var(--accent-glow))"; });
bossArt.addEventListener("mouseleave", () => { bossArt.style.filter = "none"; });
